...
api_url = "http://13.201.156.2:8000/data/"
...
def get_graph():
    ...
    for node, color, shape in zip(G.nodes(), node_colors, node_shapes):
        if shape == 's':
            nx.draw_networkx_nodes(G, layout, nodelist=[node], node_size=2500, node_color=color, node_shape='s', linewidths=2)
        else:
